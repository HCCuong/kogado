			...
			<div class="col-lg-4 col-md-4 col-sm-6 col-12">
				<div class="item_product">
					<div class="zoom_product hover-glass">
						<a href="<?= $v[$sluglang] ?>">
							<?= $func->getImage(['sizes' => '246x218x1', 'upload' => UPLOAD_PRODUCT_L, 'image' => $v['photo'], 'alt' => $v['name' . $lang]]) ?>
						</a>
					</div>
					<div class="name_product">
						...
						<?php if ($v["discount"] > 0) { ?>
							<p class=" price_sp">
								Size M: <span><?= $func->formatMoney($v['sale_price']) ?></span><br>
								Size L: <span class="i_giacu"><?= $func->formatMoney($v['regular_price']) ?></span>
							</p>
						<?php } else { ?>
							<p class=" price_sp">
								<?= ($v['regular_price'] > 0) ? 'Giá' : '' ?><span><?= ($v['regular_price'] > 0) ? $func->formatMoney($v['regular_price']) : '<a href="tel:' . preg_replace('/[^0-9]/', '', $optsetting['hotline']) . '">' . $optsetting['hotline'] . '</a>' ?></span>
							</p>
						<?php } ?>
					</div>
					<!-- <p class="cart-item addcart" data-id="<?= $v['id'] ?>" data-action="addnow"></p> -->
				</div><!--item_product-->
			</div>
		<?php } ?>
	</div>
